...
print <<"END.";
  ...
  --te         a TE liabrary (e.g. output from EDTA pipeline,TE_00003452_INT#LTR/Gypsy)                [REQUIRED]
  --cds        a cds file from this species or a closely related species     [REQUIRED]
  --out        annotated vcf file                                            [REQUIRED]
  --threads    number of threads to be used                                  [OPTIONAL]
...
&log("Step 1.Dup Typing...");
&DUP($vcf,$svtype);
&log("Step 2.Converting to Fasta Sequences...");
&convert_to_fasta("$vcf", "$vcf.fa", $svtype);
&log("Step 3.Performing Minimap2 Round 1...");
system qq (minimap2 -t $threads -c $te $vcf.fa > TE.$svtype.paf);
&log("Step 3.Performing Minimap2 Round 2...");
system qq (minimap2 -t $threads -c -x asm5 $cds $vcf.fa > CDS.$svtype.paf);
&log("Step 4.Sort Round 1...");
system qq (sort --parallel=$threads -k1,1 -k3,3n TE.$svtype.paf > TE.$svtype.sort.paf);
&log("Step 5.Sort Round 2...");
system qq (sort --parallel=$threads -k1,1 -k3,3n CDS.$svtype.paf > CDS.$svtype.sort.paf);
...
while (my $line = <$fh>) {
    ...
    my $repeat_type = $repeat_name;
    #    if ($seqtype=~/TE/) {
    #($repeat_type) = $repeat_name =~ /#(.+?)\// || "Unknown";
    #} else {
    #($repeat_type) = $repeat_name || "Unknown";
    #}
...
sub log{
    my ($input_string) = @_; # 获取传入的字符串参数
    my ($sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst) = localtime();
    my $formatted_time = sprintf('%04d-%02d-%02d %02d:%02d:%02d', 
                                  $year+1900, $mon+1, $mday, $hour, $min, $sec);
    
    print "[$formatted_time] $input_string\n"; # 打印格式化的时间和输入字符串，手动添加换行符
}
