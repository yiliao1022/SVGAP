...
my $dnafull="$library_path/EDNAFULL";
...
print "Chromosome Number: @chromosomes!!!";
...
if (exists ${$liftover_coord}{$beg0} && exists ${$liftover_coord}{$end0}) {
    ...
    $indseq = &SubSeq("$queries/$ind.$scf.fa",$begflank1[0],$endflank1[1],$begflank1[1],"minus");
    } else {
     print VCF "\.|\.:22\t$line\n";
     next;
    }
...
print SEQ2 ">$q\n$seq2\n";
# print STDERR "${stretcher}stretcher -asequence $OUTPUTdir/$q/$t.fa -bsequence $OUTPUTdir/$q/$q.fa -outfile $OUTPUTdir/$q/$t.$q.stretcher"."\n";
`${stretcher}stretcher -asequence $OUTPUTdir/$q/$t.fa -bsequence $OUTPUTdir/$q/$q.fa -outfile $OUTPUTdir/$q/$t.$q.stretcher -datafile=$dnafull`;
